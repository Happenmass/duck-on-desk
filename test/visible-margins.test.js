...
describe("visible margin envelopes", () => {
  ...
  it("prefers layout.marginBox over contentBox when present", () => {
    ...
    const contentRect = hitGeometry.getContentRectScreen(duck, bounds, "idle", idleFile, {
      box: duck.layout.contentBox,
    });
    const marginRect = hitGeometry.getContentRectScreen(duck, bounds, "idle", idleFile, {
  ...
  it("collects a non-mini envelope file set", () => {
    ...
    assert.ok(files.includes("duck-react-drag.svg"));
    assert.ok(!files.includes("duck-mini-idle.svg"));
  ...
  it("builds the update anchor from marginBox and the idle file", () => {
    ...
    assert.deepStrictEqual(computeThemeAnchorRect(duck, bounds), expected);
  });
  ...
  it("prefers updateBubbleAnchorBox over layout-derived boxes when present", () => {
    ...
    assert.deepStrictEqual(
      computeThemeAnchorRect(duck, bounds),
      hitGeometry.getContentRectScreen(duck, bounds, "idle", duck.states.idle[0], {
        box: duck.updateBubbleAnchorBox,
      })
    );
  });
