...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "duck-pi-install-"));
  tempDirs.push(dir);
  return dir;
}
...
function makeSourceDir() {
  ...
  fs.writeFileSync(path.join(dir, "pi-extension.ts"), "export default function duckPiExtension() {}\n", "utf8");
...
describe("pi-install", () => {
  ...
  it("skips registration when neither Pi config nor Pi command exists", () => {
    ...
    assert.deepStrictEqual(result, {
      ...
      updated: false,
      reason: "pi-not-found",
      extensionDir: path.join(parentDir, "extensions", "duck-on-desk"),
    });
  });
  ...
  it("does not overwrite an unmanaged existing Pi extension directory", () => {
    ...
    const sourceDir = makeSourceDir();
    const parentDir = path.join(root, ".pi", "agent");
    const extensionDir = path.join(parentDir, "extensions", "duck-on-desk");
  ...
  it("uninstalls only a Duck-managed Pi extension directory", () => {
    const root = makeTempDir();
    const sourceDir = makeSourceDir();
    const parentDir = path.join(root, ".pi", "agent");
