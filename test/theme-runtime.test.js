...
const createThemeRuntime = require("../src/shell/theme-runtime");
...
function makeFixture() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "duck-theme-runtime-"));
  tempDirs.push(tmp);
...
describe("theme-runtime active ownership", () => {
  it("keeps active theme caches out of theme-loader and deferred wrappers out of main", () => {
    const loaderSource = fs.readFileSync(path.join(SRC_DIR, "shell", "theme-loader.js"), "utf8");
  ...
  it("keeps theme-loader stateless while legacy active facades delegate to the runtime", () => {
    ...
    const duck = runtime.loadInitialTheme("duck");
    const loadedCalico = themeLoader.loadTheme("calico", { strict: true });

    assert.strictEqual(duck._id, "duck");
    assert.strictEqual(loadedCalico._id, "calico");
    assert.strictEqual(runtime.getActiveTheme()._id, "duck");
    assert.strictEqual(themeLoader.getActiveTheme()._id, "duck");
  ...
  it("loads the startup theme with requested variant and override signature", () => {
    ...
    const theme = runtime.loadInitialTheme("duck", {
      variant: "cozy",
  ...
  it("dedups an already-active theme without running the reload protocol", () => {
    ...
    runtime.loadInitialTheme("duck");

    const result = runtime.activateTheme("duck");

    assert.deepStrictEqual(result, { themeId: "duck", variantId: "default" });
  ...
  it("returns the resolved variant id when activation falls back from an unknown variant", () => {
    ...
    const result = runtime.activateTheme("duck", "missing");
  ...
  it("refreshes active theme hitbox overrides without running the full reload protocol", () => {
    ...
    const result = runtime.refreshActiveThemeHitboxOverrides("duck", {
      hitbox: {
        wide: {
          "thinking.svg": true,
        },
