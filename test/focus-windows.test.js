...
describe("Windows terminal focus", () => {
  ...
  it("requires unique title matches for Windows Terminal parent and fallback windows", () => {
    ...
    try {
      ...
      assert.match(cmd, /Get-DuckVisiblePidWindows/);
      assert.match(cmd, /Get-DuckWindowsTerminalWindows/);
  ...
  it("reports Windows helper results through stdout instead of writing logs directly", () => {
    ...
    try {
      ...
      assert.match(cmd, /Write-DuckFocusResult/);
      ...
      assert.match(helperScript, /__DUCK_FOCUS_RESULT__/);
  ...
  it("caches successful Windows focus HWNDs by session key only with matching titles", () => {
    ...
    try {
      ...
      assert.match(cmd, /\$global:DuckFocusWindowCache/);
      assert.match(cmd, /\$cacheTitleNames = @\(/);
      assert.match(cmd, /Test-DuckWindowTitleMatch/);
      assert.match(cmd, /sourcePid = \$focusCacheSourcePid/);
      assert.match(cmd, /titleNames = @\(\$cacheTitleNames\)/);
      assert.match(cmd, /Get-DuckCachedWindow/);
      assert.match(cmd, /reason = 'cached-window'/);
      assert.match(cmd, /Remove\(\$focusCacheKey\)/);
      assert.match(cmd, /Save-DuckFocusCache \$matches\[0\]/);
      assert.match(cmd, /Save-DuckFocusCache \$wtMatches\[0\]/);
      assert.match(cmd, /Save-DuckFocusCache \$wtHwndFromHook/);
      assert.doesNotMatch(cmd, /Save-DuckFocusCache \$pidWindows\[0\]/);
      assert.doesNotMatch(cmd, /Save-DuckFocusCache \$singleWtWindows\[0\]/);
      assert.doesNotMatch(cmd, /Save-DuckFocusCache \$pendingConsoleHwnd/);
    } finally {
      cleanup();
    }
  });
  ...
  it("holds the focus result until the Orca pane switch has answered", async () => {
    ...
    try {
      ...
      stdout.emit("data", `__DUCK_FOCUS_RESULT__ {"token":"${token}","reason":"orca-window","targetHwnd":"787468","foregroundHwnd":"787468","confirmed":true,"status":"confirmed"}\n`);
  ...
  it("does not make non-Orca sessions wait on a pane switch", async () => {
    ...
    try {
      ...
      const token = writes[0].match(/\$focusToken = '([^']+)'/)[1];
      stdout.emit("data", `__DUCK_FOCUS_RESULT__ {"token":"${token}","reason":"parent-direct","targetHwnd":"111","foregroundHwnd":"111","confirmed":true,"status":"confirmed"}\n`);
  ...
  it("correlates concurrent Windows helper results by token", async () => {
    ...
    try {
      ...
      stdout.emit("data", `__DUCK_FOCUS_RESULT__ {"token":"${tokenB}","reason":"parent-direct","targetHwnd":"222","foregroundHwnd":"222","confirmed":true,"status":"confirmed"}\n`);
      ...
      stdout.emit("data", `__DUCK_FOCUS_RESULT__ {"token":"${tokenA}","reason":"wt-title-ambiguous","targetHwnd":"111","foregroundHwnd":"111","confirmed":true,"status":"confirmed"}\n`);
  ...
  it("accepts options-object requests and redacts full cwd in focus logs", () => {
    ...
    const { initFocus, cleanup } = loadFocusWithMock({
      execFile: (cmd, args, opts, cb) => {
        ...
        if (cb) cb(null, "__DUCK_FOCUS_RESULT__ parent-direct\n", "");
      },
      spawn: () => ({
        pid: 9999,
        stdin: {
