...
const client=new Client({name:'foot-recovery-check',version:'1.0.0'}),report={jobs:[]};
...
try{
 ...
 let d=await call('lab_experiment_create',{name:'允许失稳后蹬地恢复：连续采样与续训核验',mode:'robot',task:'microduck-headstand-hold'});
 ...
 assert.equal(parent.metrics.reduce((n,m)=>n+m.timeout_resets,0),0);
 ...
 assert.equal(child.manifest.iterations,12);assert.equal(child.manifest.reset_on_pose_loss,false);
 assert.equal(child.metrics.reduce((n,m)=>n+m.failure_resets,0),0);
 assert.equal(child.metrics.reduce((n,m)=>n+m.timeout_resets,0),0);
 report.ok=true;save();console.log(JSON.stringify({ok:true,parent:parent.id,child:child.id,candidate:child.manifest.candidate}));
