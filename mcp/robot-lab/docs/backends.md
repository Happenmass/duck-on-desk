...
Managed dependencies: Python 3.12, torch 2.9.1, numpy 2.4.1, mujoco 3.10.0, onnx 1.22.0, onnxruntime 1.24.4. Windows x64 CUDA installs the cu128 wheel; Apple Silicon installs the macOS wheel with MPS; an explicitly selected CPU backend uses CPU wheels. The preparer checks imports and an actual gradient operation on every requested device before training. It does not install NVIDIA drivers, and never substitutes CPU for a requested unavailable GPU. Intel macOS and Windows ARM require a custom Python environment for this recipe.
