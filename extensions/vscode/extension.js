...
// Port range for Duck terminal-focus extension instances.
...
function tryListen(port, maxPort) {
  if (port > maxPort) {
    console.log("Duck terminal-focus: all ports in use, HTTP server disabled");
    return;
  }
  ...
  server.listen(port, "127.0.0.1", () => {
    boundPort = port;
    console.log(`Duck terminal-focus: listening on 127.0.0.1:${port}`);
  });
}
...
function activate(context) {
  ...
  // vscode://duck.duck-terminal-focus?pids=1234,5678
