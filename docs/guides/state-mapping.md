...
| roam (free roam; the duck moves the window) | `duck-roam` | Walk at the edge of the camera-facing cone toward the side given by main's `roam-heading` (heading ±60°, forward 0.6; the runtime clamps every heading request to ±60°, so the duck never shows its back). The renderer reports how far the trunk actually walked (screen px, `duck-displacement`); main slides the window by the lateral part of that stride and adds the walk's planned up/down drift in step with it, so window motion and gait always agree. Main picks the side from the duck's reported facing (the side it already leans to), skips the walk when that side has no room, and ends it once the planned distance is covered or the walk is blocked at an edge |
