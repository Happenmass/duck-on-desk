...
const MARKER_FILENAME = ".duck-on-desk-codex-pet.json";
...
function generateWrapperSvg({ rowKey, mode, spritesheetHref, atlas = ATLAS }) {
  ...
  return [
    `<!-- Generated by Duck codex-pet-adapter v${ADAPTER_VERSION}: ${row.key} ${mode}. -->`,
...
function buildUniversalVisualStyle(initialSpec, atlas) {
  ...
  for (const spec of UNIVERSAL_VISUAL_SPECS) {
    ...
    const rowOffsetY = row.row * atlas.frameHeight;
    lines.push(
      `[data-duck-codex-pet-visual=${spec.visual}] .visual-row {`,
      `  transform: ${formatTranslate(0, rowOffsetY)};`,
      "}"
    );
    if (spec.mode === "static") {
      lines.push(
        `[data-duck-codex-pet-visual=${spec.visual}] .atlas {`,
        "  animation-name: none;",
        "}"
      );
      continue;
  ...
  lines.push(
    "[data-duck-codex-pet-visual=drag-directional][data-duck-drag-direction=left] .visual-row {",
...
function generateUniversalWrapperSvg({ initialFilename, spritesheetHref, atlas = ATLAS }) {
  ...
  return [
    `<!-- Generated by Duck codex-pet-adapter v${ADAPTER_VERSION}: universal visual bridge (${initialSpec.visual}). -->`,
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${atlas.frameWidth} ${atlas.frameHeight}" width="${atlas.frameWidth}" height="${atlas.frameHeight}" data-duck-codex-pet-visuals="v1" data-duck-codex-pet-visual="${initialSpec.visual}" data-duck-drag-directional="v1" data-duck-drag-direction="right">`,
    "  <defs>",
    "    <clipPath id=\"codex-pet-frame\">",
    `      <rect x="0" y="0" width="${atlas.frameWidth}" height="${atlas.frameHeight}"/>`,
    "    </clipPath>",
...
function validatePngAtlasAlpha(idatChunks, info, atlas) {
  ...
  // containing the 16 clockwise gaze directions. Duck does not consume those
