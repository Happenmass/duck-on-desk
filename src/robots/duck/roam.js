...
// src/robots/duck/roam.js — Free roam mode: pet wanders around the desk when idle
//
...
//   • Optional roam fence (#810): when ctx.roamFence (src/robots/duck/roam-fence.js)
...
module.exports = function initRoam(ctx) {
  ...
  function isRoamAllowed() {
    ...
    // duck-on-desk: a pet body without a mobile base (Reachy Mini) never roams.
    if (ctx.canRoam && !ctx.canRoam()) return false;
  ...
  function pickRandomTarget() {
    ...
    // src/robots/duck/roam-fence.js; refreshed asynchronously in scheduleNextRoam), never
  ...
  function tick() {
    if (!enabled) return;
    if (!isRoamAllowed()) {
      dbg("blocked", { dragLocked: ctx.dragLocked, mini: ctx.getMiniMode && ctx.getMiniMode(), canRoam: !ctx.canRoam || ctx.canRoam(), state: ctx.getCurrentState && ctx.getCurrentState(), bubble: hasPermissionBubbleHold() });
