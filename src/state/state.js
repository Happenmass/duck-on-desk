// src/state/state.js — State machine + session management + DND + wake poll
...
function applyState(state, svgOverride, options = {}) {
  ...
  // Publish the committed semantic state independently from its visual
  // projection. Physical adapters consume this high-level intent directly;
  // a missing, reloading, or wedged renderer must never block robot control.
  if (typeof ctx.onSemanticState === "function") {
    try {
      ctx.onSemanticState(state, { ...applyOptions, previousState });
    } catch {}
  }
...
function wakeRobot() {
  stopWakePoll();
  ctx.mouseStillSince = Date.now();
  if (ctx.doNotDisturb) disableDoNotDisturb();
  else playWakeTransitionOrResolve();
}
...
return {
  ...
  shouldDropForDnd,
  enableDoNotDisturb, disableDoNotDisturb, wakeRobot,
