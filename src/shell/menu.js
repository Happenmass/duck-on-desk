...
// src/shell/login-item.js + main.js's settings-actions effect. menu.js used to
...
// i18n string pool + translator factory live in src/shell/i18n.js so the future
...
module.exports = function initMenu(ctx) {
  ...
  function buildDuckAppearanceMenuItems() {
    ...
    return [
      {
        label: t("menuPetRobot"),
        submenu: [["duck", "Microduck"], ["reachy-mini", "Reachy Mini"]].map(([id, label]) => ({
          label, type: "radio", checked: (settings.get("petRobot") || "duck") === id,
          click: () => settings.applyUpdate("petRobot", id),
        })),
      },
      ...(settings.get("petRobot") === "reachy-mini" ? [{
        label: t("menuReachyConnect"), type: "checkbox", checked: settings.get("reachyAutoConnect") !== false,
        click: item => settings.applyUpdate("reachyAutoConnect", item.checked),
      }, {
  ...
  function buildTrayMenu() {
    ...
    const stateGroup = [
      {
        label: (ctx.isRobotSleeping?.() ?? ctx.doNotDisturb) ? t("wake") : t("sleep"),
        enabled: !ctx.isRobotWaking?.(),
        click: () => (ctx.isRobotSleeping?.() ?? ctx.doNotDisturb)
          ? (ctx.wakeRobot ? ctx.wakeRobot() : ctx.disableDoNotDisturb()) : ctx.enableDoNotDisturb(),
      },
    ];
