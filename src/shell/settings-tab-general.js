...
(function initSettingsTabGeneral(root) {
  ...
  function buildRobotGroup() {
    const row = document.createElement("div"); row.className = "row";
    row.innerHTML = '<div class="row-text"><span class="row-label"></span></div><div class="row-control"></div>';
    row.querySelector(".row-label").textContent = t("rowPetRobot");
    const save = async (key, value) => {
      try {
        const result = await window.settingsAPI.update(key, value);
        if (result?.status === "ok") return true;
        ops.showToast(t("toastSaveFailed") + (result?.message || ""), { error: true });
      } catch (err) { ops.showToast(t("toastSaveFailed") + err.message, { error: true }); }
      return false;
    };
    const select = helpers.buildSettingsSelect({
      value: state.snapshot?.petRobot || "duck", ariaLabel: t("rowPetRobot"),
      options: [{ value: "duck", label: "Microduck" }, { value: "reachy-mini", label: "Reachy Mini" }],
      onChange: value => save("petRobot", value),
    });
    row.querySelector(".row-control").appendChild(select.element);
    const hostRow = document.createElement("div"); hostRow.className = "row";
    hostRow.innerHTML = '<div class="row-text"><span class="row-label"></span><span class="row-desc"></span></div><div class="row-control"><input type="text" class="text-input" spellcheck="false"></div>';
    hostRow.querySelector(".row-label").textContent = t("rowReachyHost");
    hostRow.querySelector(".row-desc").textContent = t("rowReachyHostDesc");
    const input = hostRow.querySelector("input");
    input.value = state.snapshot?.reachyHost || "";
    input.placeholder = "reachy-mini.local:8000";
    input.setAttribute("aria-label", t("rowReachyHost"));
    input.addEventListener("change", () => save("reachyHost", input.value.trim()));
    return helpers.buildCollapsibleGroup({
      id: "general:robot", title: t("rowPetRobot"), desc: t("rowPetRobotDesc"), defaultCollapsed: false,
      children: [row,
        helpers.buildSwitchRow({ key: "reachyAutoConnect", labelKey: "rowReachyConnect", descKey: "rowReachyConnectDesc" }),
        hostRow,
      ],
    });
  }
  ...
  // Mirrors TEXT_SCALE_MIN/MAX/STEP in src/shell/text-scale.js (×100). The renderer
  ...
  // = prefsSizeToUi(9): the prefs `size` default is "P:9" (see src/shell/prefs.js).
