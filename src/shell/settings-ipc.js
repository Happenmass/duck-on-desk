...
const { detectAgentInstallations: defaultDetectAgentInstallations } = require("../state/agent-installation-detector");
  ...
  handle("settings:detect-agent-installations", async (_ev, opts) => {
    try {
      ...
      if (options.refreshWsl) {
        const { refreshWslDetection } = require("../state/agent-installation-detector");
