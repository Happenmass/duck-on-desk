...
// src/shell/display-edge.js — shared horizontal display-edge topology resolver.
//
// Both src/shell/mini.js's internal-seam clip (`seamBoundary()`) and the Linux
// edge-virtualization materializer (`materializeVirtualBounds()` in
// src/shell/drag-position.js, wired up in a later phase) need the same answer to
...
// the tolerance src/shell/mini.js's `seamBoundary()` already used.
...
// the caller's workArea. Mirrors src/shell/mini.js's previous inline lookup.
