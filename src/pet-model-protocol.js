...
function resolvePolicyRequest(url, { homeDir = os.homedir(), exists = fs.existsSync } = {}) {
  ...
  if (!POLICY_NAMES.has(name)) return { status: 404 };
  const file = path.join(policyDirectory(homeDir), name);
...
module.exports = { SCHEME, POLICY_COMMIT, POLICY_NAMES, policyDirectory, resolvePolicyRequest, registerScheme, installHandler };
