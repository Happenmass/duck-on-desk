...
const evidence=path.join(repo,'.release-reviews/resume-training');
const records=JSON.parse(fs.readFileSync(path.join(evidence,'mcp.json'))).jobs;
const parent=records.find(j=>j.phase==='cancelled');
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'duck-resume-ui-'));
...
app.whenReady().then(async()=>{
 ...
 try{
  jobs=require(path.join(labRoot,'jobs.cjs')).createLabJobs({root:path.join(evidence,'jobs')});
  ...
  fs.writeFileSync(path.join(output,'resume.png'),(await wc.capturePage()).toPNG());
