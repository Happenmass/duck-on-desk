param(
  ...
  [string]$Pattern = 'Duck on Desk|duck-on-desk|src[\\/]+main\.js',
  [int[]]$RootPid,
  [string]$OutputPath,
  [switch]$Once,
  [switch]$Quiet
...
if (-not $OutputPath) {
  ...
  $OutputPath = Join-Path "logs" "duck-power-$timestamp.csv"
}
...
if (-not $Quiet) {
  Write-Host "Monitoring Duck power/resource proxy metrics."
