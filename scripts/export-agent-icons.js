...
const EXPORTER_ENV = "DUCK_AGENT_ICON_EXPORTER";
...
function main() {
  ...
  try {
    if (!dryRun) stagingDir = fs.mkdtempSync(path.join(os.tmpdir(), "duck-agent-icon-outputs-"));
    for (const agent of agents) {
...
function runInElectron() {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "duck-agent-icons-"));
